...
class SiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        visual_tokens = visual_tokens + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm2(visual_tokens), shift_mlp, scale_mlp))
...
class SiT(nn.Module):
    ...
    def forward(self, x, t, y, 
        ...
        for i, block in enumerate(self.blocks):
            #print("koko")
            #x = block(x, c)                      # (N, T, D)
            combined_embeddings_try_2 = block(combined_embeddings_try_2, c)
            #print("1 3 2 x")
            if (i + 1) == self.encoder_depth:
                ...
                zs = [projector(combined_embeddings_try_2[:,:T,:].reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
                #zs = [projector(combined_embeddings[:, :T].reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
        #print("1 3 2 1")
        x = combined_embeddings_try_2[:,:T,:]
