...
class SiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        T = x.shape[1] - NUM_OF_CRITERIA[self.task]
        latent_tokens = x[:, :T, :]
        #print(f"latent_tokens shape: {latent_tokens.shape}")
        visual_tokens_crit = x[:, T:T+NUM_OF_CRITERIA[self.task], :]
...
class SiT(nn.Module):
    ...
    def forward(self, x, t, y, 
        ...
        #print("1 3 3")
        return x,image_embed, minus_embedded_concepts_plus_noise, combined_embeddings_try[:,T:T+NUM_OF_CRITERIA[self.task],:], y_predicted_logits.squeeze(1), zs
