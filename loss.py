...
class SILoss:
    ...
    def __call__(self, model, images, model_kwargs=None, zs=None, labels=None, explicid=None, explicid_imgs_list= None, epoch=None,  explicd_only=0):
        ...
        for imgs_indx, explicid_imgs in enumerate(explicid_imgs_list):
            ...
            if  explicd_only==0:
                if epoch<2:
                    model_output, image_embed, embedded_concepts, produced_concepts, y_predicted, sparsity_loss, zs_tilde  = model(model_input, model_input_pure, model_target, time_input.flatten(), **model_kwargs, 
                                                                        concept_label=concept_label, 
                                                                        image_embeddings=agg_visual_tokens,
                                                                        cls_logits=cls_logits,
                                                                        con_on_explicd_pred=False )
                else:
                    model_output, image_embed, embedded_concepts, produced_concepts, y_predicted, sparsity_loss, zs_tilde  = model(model_input, model_input_pure, model_target,time_input.flatten(), **model_kwargs, 
                                                                            concept_label=concept_label, 
                                                                            image_embeddings=agg_visual_tokens,
        ...
        if  explicd_only==0:
            return denoising_loss, proj_loss, explicid_loss, loss_cls, logits_similarity_loss, cosine_loss, sit_cls_loss, contr_loss, loss_cls_criteria_only, loss_cls_refined, sparsity_loss
        else:
            return None, None, explicid_loss, loss_cls, logits_similarity_loss, None, None, None, None
