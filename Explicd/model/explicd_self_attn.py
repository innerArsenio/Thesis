...
class ExpLICD_ViT_L(nn.Module):  
    def __init__(self, concept_list, model_name='biomedclip', config=None):
        ...
        self.visual_tokens = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(NUM_OF_CRITERIA[self.task], 1024, dtype=torch.float32)))
        ...
        #                                     34  for ISIC
        #                                     17 for BUSI
        self.cls_head = nn.Linear(in_features=18, out_features=config.num_class)
    ...
    def forward(self, imgs, refined_tokens=None):
        ...
        agg_visual_tokens_for_explicid = self.proj(self.norm(self.ffn(agg_visual_tokens[:,:NUM_OF_CRITERIA[self.task],:])))
        ...
        return cls_logits, None, image_logits_dict,  F.normalize(agg_visual_tokens_for_SiT, dim=-1)
