...
class ExpLICD(nn.Module):  
    def __init__(self, concept_list, model_name='biomedclip', config=None):
        ...
        if self.model_name in ['biomedclip', 'openclip']:
            ...
            #self.soft_prompt = nn.Parameter(torch.randn(1, 10, 512))  # Adjust dimensions as needed
    ...
    def forward(self, imgs):
        ...
        agg_visual_tokens_explic = self.proj(self.norm(self.ffn(agg_visual_tokens)))
        
        agg_visual_tokens_explic = F.normalize(agg_visual_tokens_explic, dim=-1)

        image_logits_dict = {}
        idx = 0
        for key in self.concept_token_dict.keys():
            image_logits_dict[key] = (self.logit_scale * agg_visual_tokens_explic[:, idx:idx+1, :] @ self.concept_token_dict[key].repeat(B, 1, 1).permute(0, 2, 1)).squeeze(1)
            idx += 1
        ...
        return cls_logits, image_logits_dict, agg_visual_tokens
