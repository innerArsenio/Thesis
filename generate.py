...
def main(args):
    ...
    for i in pbar:
        ...
        if using_cfg:
            z = torch.cat([z, z], 0)
            y_null = torch.tensor([7] * n, device=device)
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(samples):
                ...
                class_index = y[i].item() % len(class_names)  # Get class index
                class_name = class_names[class_index]  # Get class name
                Image.fromarray(sample).save(f"{sample_folder_dir}/{class_name}_{index:06d}.png")
